...
// Email Handler
     var sendfeedbackMail= function(emailAddress ,username ,newComment) {
        ...
        var transporter = nodemailer.createTransport({
          ...
          mystring =  mystring + '<td>' +  newComment.firstname + '</td>'     
          mystring =  mystring + '</tr>'
          mystring =  mystring + '<tr>'
          mystring =  mystring + '<td>LastName</td>'
          mystring =  mystring + '<td> ' + newComment.lastname + '</td>'   
          mystring =  mystring + '</tr>'
          mystring =  mystring + '<tr>'
          mystring =  mystring + '<td>Phone number</td>'
          mystring =  mystring + '<td> ' + newComment.phonenumber + '</td>'  
          mystring =  mystring + '</tr>'
          mystring =  mystring + '<tr>'
          mystring =  mystring + '<td>Email Address</td>'
          mystring =  mystring + '<td> ' + newComment.email + '</td>'   
          mystring =  mystring + '</tr>'
          mystring =  mystring + '<tr>'
          mystring =  mystring + '<td>Comment</td>'
          mystring =  mystring + '<td> ' + newComment.comment + '</td>'   
          mystring =  mystring + '</tr>'
        ...
        var mailContent = {
            ...
            subject: 'Comment from mobile application',
            ...
            //         <tr>
            //                 <td>First Name</td>
            //                 <td>' +  newcomment.firstName + '</td>     
            //               </tr>
            //               <tr>
            //                 <td>LastName</td>
            //                 <td> ' + newcomment.lastName + '</td>    
            //               </tr>
            //               <tr>
            //                 <td>Phone number</td>
            //                 <td> ' + newcomment.phonenumber + '</td>   
            //               </tr>
            //              <tr>
            //                 <td>Email Address</td>
    ...
    var sendMail= function(emailAddress ,username) {
        ...
        var mailContent = {
            from: 'no.reply.harrisconsult@gmail.com',
            to: emailAddress,
            subject: 'Your comment post',
            text: 'Your comment post',
            html: 'Hello ' + username + ' , <br/> Thank you for leaving your feedback. <br> Your comment will be view and we will get back to you.'
        };
...
exports.create = function(req, res) {
  ...
  var comment_firstname = req.body.firstname;
  var comment_lastname = req.body.lastname;
  var comment_email = req.body.email;
  var comment_phonenumber = req.body.phonenumber;
  var comment_comment= req.body.comment;


 comment.findOne({ time: '2013-10-01T00:00:00.000Z' },
function(err, doc) { // Using RegEx - search is case insensitive
    if(!err && !doc) {
      
      console.log ("There was a request");
      var newCommment = new comment();
      
      newCommment.firstname = comment_firstname;
      newCommment.lastname = comment_lastname;
      newCommment.email = comment_email;
      newCommment.phonenumber = comment_phonenumber;
      newCommment.comment = comment_comment; //cc.generate({ parts : 2 });
      ...
      newCommment.save(function(err) {
        ...
        if(!err) {
          ...
          sendfeedbackMail(comment_email , comment_firstname , newCommment);
          sendMail(comment_email , comment_firstname , comment_comment);

        } else {
          res.json(500, {status: "Comment not sent. Error: " + err});
        }
